...
def pair_indices(messages: list[dict[str, Any]]) -> list[tuple[int, int]]:
    ...
    for result_index in range(1, len(messages)):
        action_index = result_index - 1
        if messages[action_index].get("role") != "assistant" or messages[result_index].get("role") != "user":
            continue
        try:
            action = json.loads(messages[action_index]["content"])
            envelope = json.loads(messages[result_index]["content"])
        except (KeyError, TypeError, json.JSONDecodeError):
            continue
        if not isinstance(action, dict) or not isinstance(envelope, dict) or "result" not in envelope:
            continue
