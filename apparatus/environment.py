...
def validate_action(cell: str, action: dict[str, Any]) -> dict[str, Any]:
    ...
    schema = json.loads((ROOT / "parent_evidence" / "schema" / "action-schema.json").read_text(encoding="utf-8"))
    candidates = [
        item
...
class ActionEnvironment:
    ...
    def __init__(self, cell: str, candidate_root: Path, submission_root: Path) -> None:
        ...
        fixture_candidate = ROOT / "parent_evidence" / "candidate"
    ...
    def _build_reopen_map(self) -> dict[str, dict[str, Any]]:
        ...
        def add_messages(messages: list[dict[str, Any]]) -> None:
            for result_index, result_message in enumerate(messages):
                if result_index == 0 or result_message.get("role") != "user":
                    continue
                ...
                if not isinstance(result, dict) or "context_receipt" in result:
                    continue
                results[compact_json(action)] = copy.deepcopy(result)

        for relative in (
            "parent_evidence/schema/historical-turn-010-request.json",
            "parent_evidence/boundary/call-12-request.json",
        ):
            add_messages(json.loads((ROOT / relative).read_text(encoding="utf-8"))["messages"])
        return results
