...
from apparatus.runner import get_json, require_authorization, require_clean_head, run_experiment, verify_endpoint, verify_runtime_files
...
def llama_processes() -> list[str]:
    process = subprocess.run(["tasklist", "/FI", "IMAGENAME eq llama-server.exe", "/FO", "CSV", "/NH"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False, text=True)
    return [line.strip() for line in process.stdout.splitlines() if "llama-server.exe" in line.lower()]
...
def server_arguments(server: Path, model: Path, host: str, port: int, runtime: Path) -> list[str]:
    ...
    return [
        str(server), "-m", str(model), "--alias", "qwen38-ad25q8-world-join",
        "--host", host, "--port", str(port), "--gpu-layers", "all", "--fit", "off",
        "-c", "25000", "--flash-attn", "on", "-ctk", "q8_0", "-ctv", "q8_0",
        "--kv-unified", "-b", "512", "-ub", "256", "--threads", "7",
        "--threads-batch", "8", "--parallel", "1", "--cache-prompt", "--cache-ram", "0",
        "--slot-save-path", str(slots), "--no-context-shift", "--jinja", "--reasoning", "off",
        "--reasoning-format", "deepseek", "--reasoning-budget", "0", "--no-reasoning-preserve",
        "--temp", "0.7", "--top-p", "0.8", "--top-k", "20", "--min-p", "0.0",
        "--presence-penalty", "1.5", "--repeat-penalty", "1.0", "--metrics", "--slots",
        "--no-webui", "--no-mmproj", "--verbose", "--log-file", str(runtime / "llama-server.log"),
    ]
...
def execute(run_id: str, server: Path, model: Path, host: str, port: int) -> dict[str, Any]:
    ...
    package = verify_runtime_files(server.resolve(), model.resolve())
    if llama_processes():
        raise RuntimeError("llama-server isolation check failed")
    if port_open(host, port):
        raise RuntimeError(f"port already open: {host}:{port}")
    ...
    lifecycle: dict[str, Any] = {
        "schema_version": "lossy-source-digest-runtime-lifecycle-v0", "study_id": STUDY_ID,
        "run_id": run_id, "standalone_commit": head, "started_at_unix": time.time(),
        "server_arguments": arguments, "gpu_before": gpu_state(), "passed": False,
    }
    ...
    try:
        with (runtime / "stdout.log").open("wb") as stdout, (runtime / "stderr.log").open("wb") as stderr:
            ...
            offloads = [(int(a), int(b)) for a, b in re.findall(r"offloaded\s+(\d+)/(\d+)\s+layers to GPU", log)]
            main = next(((a, b) for a, b in reversed(offloads) if b == 66), None)
            lifecycle["main_offloaded_layers_before_calls"] = list(main) if main else None
            if main != (66, 66):
                raise RuntimeError("exact CUDA profile did not offload 66/66 layers")
            props = verify_endpoint(base_url, model.resolve())
            custody = {
                "schema_version": "lossy-source-digest-runtime-custody-v0", "server_and_model": package,
                "server_arguments": arguments, "endpoint_props": props,
            ...
            result = run_experiment(run_id, base_url, custody)
            lifecycle["run_result"] = result
            lifecycle["passed"] = not result["apparatus_invariant_failure"] and result["model_calls"] <= MAXIMUM_MEASURED_CALLS
